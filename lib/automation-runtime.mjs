...
export function validateAutomationPolicy(policy) {
  ...
  if (!Number.isInteger(policy.maxApplicationsPerDay) || policy.maxApplicationsPerDay < 1) add('POLICY_INVALID', 'maxApplicationsPerDay must be a positive integer');
  if (policy.maxApplicationsPerHour != null && (!Number.isInteger(policy.maxApplicationsPerHour) || policy.maxApplicationsPerHour < 1)) add('POLICY_INVALID', 'maxApplicationsPerHour must be a positive integer');
...
export function createAutomationController(options) {
  ...
  async function runCycle(policy) {
    ...
    if (usedToday >= policy.maxApplicationsPerDay) {
