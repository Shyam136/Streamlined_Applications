...
export function createPlaywrightApplicationPort(options = {}) {
  ...
  async function inspect(rawUrl) {
    ...
    if (fieldCount === 0) {
      const applyLinks = activePage.locator('a:visible').filter({ hasText: /^(apply|apply now|apply for this job|start application)$/i });
      if (await applyLinks.count() === 1) {
        const href = await applyLinks.first().getAttribute('href');
        if (href) {
          const applyUrl = safeUrl(new URL(href, activePage.url()).href, allowedHosts);
